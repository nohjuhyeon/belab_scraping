...
def email_push(notice_list,user_type):
    ...
    if len(notice_list)> 0:
        html_content = '<h2>나라장터에 새로 올라온 공고가 있습니다. 확인 부탁드립니다.</h2>'
        for i in class_list:
            ai_notice_list = notice_list.loc[(notice_list['notice_class']==i)&(notice_list['type'].str.contains('인공지능'))]
...
def email_sending():
    ...
    if len(df)>0:
        ...
        notice_list = df.loc[df['type'].str.contains('인공지능')]
        if len(notice_list)>0:
